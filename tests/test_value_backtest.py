...
def test_value_backtest_settles_chronological_full_kelly_paper_stakes():
    ...
    assert result.to_dict()["summary"] == {
        ...
        "profit_loss": 36.57,
        "roi": 0.3657,
        "bankroll_peak": 136.57,
        "bankroll_trough": 100.0,
...
def test_value_backtest_paper_bets_include_audit_trail_and_bankroll_path():
    class FakeQueries:
        def get_value_backtest_matches(self):
            return [
                {
    ...
    paper_bets = result.to_dict()["paper_bets"]
    assert paper_bets == [
        {
            "match_id": 1,
            ...
            "implied_probability": 0.3846,
            "odds": 2.6,
            "expected_value": 0.3,
            "kelly_fraction": 0.1875,
            "stake": 18.75,
            "bankroll_before_match": 100.0,
            "bankroll_before_settlement": 100.0,
            "bookmaker": "Book A",
            "actual_outcome": "Draw",
            "result": "loss",
            "profit_loss": -18.75,
            "bankroll_after_settlement": 109.26,
        },
        {
            "match_id": 1,
            "start_time": "2026-05-29T20:00:00",
    ...
    assert "Bankroll Peak: 109.26" in report
    assert "Bankroll Trough: 100.00" in report
    assert "Model Probability: 50.0%; Implied Probability: 38.5%" in report
...
def test_value_backtest_settles_home_draw_and_away_from_final_scores():
    class FakeQueries:
        def get_value_backtest_matches(self):
            return [
                {
                ...
                },
                {
                    ...
                    "away_score": 1,
                    "prediction": {
                        "prediction_date": datetime(2026, 5, 30, 10, 0),
                        "prob_home_win": 0.10,
                        "prob_draw": 0.75,
                        "prob_away_win": 0.15,
                    },
                    "odds": [
                        {
                            ...
                            "odds_value": 2.00,
                            "bookmaker_name": "Book A",
                            "retrieved_at": datetime(2026, 5, 30, 9, 0),
                        }
                    ],
                },
                {
                    "match_id": 3,
                    "start_time": datetime(2026, 5, 31, 20, 0),
                    "home_team": "Spurs",
                    "away_team": "Fulham",
                    "home_score": 0,
                    "away_score": 2,
                    ...
                    "odds": [
                        {
                            ...
                            "odds_value": 2.00,
                            "bookmaker_name": "Book A",
                            "retrieved_at": datetime(2026, 5, 31, 9, 0),
                        }
                    ],
                },
            ]
    ...
    assert [
        (bet["outcome"], bet["actual_outcome"], bet["result"])
        for bet in result.to_dict()["paper_bets"]
    ] == [
        ("Home Win", "Home Win", "win"),
        ("Draw", "Draw", "win"),
        ("Away Win", "Away Win", "win"),
    ]


def test_value_backtest_summary_uses_bankroll_path_and_paper_bet_averages():
    class FakeQueries:
        def get_value_backtest_matches(self):
            return [
                {
    ...
    assert summary["bankroll_peak"] == 100.0
    assert summary["bankroll_trough"] == 50.0
    assert summary["max_drawdown"] == 0.5
    assert summary["average_odds"] == 2.0
    assert summary["average_expected_value"] == 0.5
    assert summary["final_bankroll"] == 75.0


def test_value_backtest_same_match_paper_stakes_use_pre_match_bankroll():
    class FakeQueries:
        def get_value_backtest_matches(self):
            return [
                {
    ...
    assert [
        (
            bet["outcome"],
            bet["stake"],
            bet["bankroll_before_settlement"],
            bet["bankroll_after_settlement"],
            bet["profit_loss"],
        )
        for bet in result.to_dict()["paper_bets"]
    ] == [
        ("Home Win", 50.0, 100.0, 100.0, -50.0),
        ("Draw", 50.0, 100.0, 100.0, 50.0),
    ]
...
def test_value_backtest_cli_runs_default_report(monkeypatch, capsys):
    ...
    assert "Bankroll Peak: 140.00" in output
    assert "Bankroll Trough: 90.00" in output
